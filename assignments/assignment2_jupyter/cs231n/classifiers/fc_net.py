...
#         print(scores)
        ...
        # loss
        N = X.shape[0]
        loss = (1/N) * np.sum(-np.log(probs[range(N), y]))
        loss += (1/2) * self.reg *  \
                        (np.sum(W1 * W1) + 
                         np.sum(W2 * W2))
...
#         print("Hidden Layer: ",hidden_layer.shape)
        ...
        dhidden_layer = (hidden_layer > 0) * np.dot(probs, W2.T)
        
#         print("dw2: ", dw2.shape)
#         print("db2: ", db2.shape)
#         print("dHidden Layer: ",dhidden_layer.shape)
        
        dw1 = np.dot(X.T, dhidden_layer)
        db1 = dhidden_layer.sum(axis=0)
...
#         self.params['W3'] = np.random.normal(0,weight_scale,(hidden_dims[1],num_classes))
#         self.params['b3'] = np.zeros((num_classes,))        
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            x, cache =  affine_norm_relu_forward(x,w,b)
            caches.append(cache)
        ...
        for i in range(self.num_layers - 2, -1, -1):
            dx, dw, db = affine_norm_relu_backward(dout, caches[i])
